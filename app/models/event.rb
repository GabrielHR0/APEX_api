class Event < ApplicationRecord
    ...
    def reorder_positions
        ...
        if old_position.nil?
        Event.where("position >= ?", new_position)
                    .where.not(id: id)
        ...
        # Movendo para baixo
        Event.where("position > ? AND position <= ?", old_position, new_position)
                    .where.not(id: id)
                    .update_all("position = position - 1")
        else
        # Movendo para cima
        Event.where("position >= ? AND position < ?", new_position, old_position)
                    .where.not(id: id)
                    .update_all("position = position + 1")
        end
    end
    ...
    def reorder_after_destroy
        ...
        Event.where("position > ?", position_to_fill)
                    .update_all("position = position - 1")
    end
